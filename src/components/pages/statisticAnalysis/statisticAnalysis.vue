...
<div class="statistic-analysis">
  <el-collapse v-model="activeNames" @change="handleChangeCollapse">
    ...
    <el-collapse-item title="结果" name="2" class="table-container opened">
      ...
      <v-table ref="resultTable" v-loading="tableLoading" :element-loading-text="tableLoadingText" :tableData="tableData" @removeCompany="handleRemoveCompany" @removeMethod="handleRemoveMethod" @showDetail="handleShowDetail" @changeCompany="handleChangeCompany"
        @showCompanyChart="handleShowCompanyChart" @clearData="handleClearData" @showMethodChart="handleShowMethodChart" @changeLevel="handleChangeLevel" @changeYear="handleChangeYear" @drillingData="handleDrillData" @exportDataByMethod="handleExportDataByMethod"
        @exportDataByCell="handleExportDataByCell" @exportDoubtData="handleExportDoubtData" @exportStatisticData="handleExportStatisticData" @exportDataByCompany="handleExportDataByCompany" @showMethodChartMulti="handleShowMethodChartMulti"></v-table>
    </el-collapse-item>
  </el-collapse>
  ...
  <el-dialog title="图表" :visible.sync="showCompanyChart" size="large" class="chart-data">
    ...
    <v-chart :chartData="companyChartData" defaultType="pie" :level="chartLevel" @click="handleClickCompanyChart" @search="handleSearchAnotherCompany"></v-chart>
    <span slot="footer" class="dialog-footer">
...
export default {
  ...
  methods: {
    ...
    loadMethodTree() {
      ...
      this.$http.post(urlStore.findAllListWithAuditItemAndAuditMethodModel)
        .then(res => {
          if (res.ok) {
    ...
    loadCompanyList() {
      ...
      this.$http.get(urlStore.findUnitsList)
        .then(res => {
          if (res.ok && res.body.success) {
            this.companyList = res.body.data
            this.selectedCompany = res.body.data.filter(item => item.unitLevel == 1)
              .slice(0, 3)
    ...
    getResult() {
      ...
      this.$http.get(urlStore.statisticQuery, {
          params: {
            ...
            unitCodes: unitCodes,
            level: level,
            methodIds: this.selectedMethods.map(item => item.id)
              .join(',')
          }
        })
        .then(res => {
      ...
      this.$http.get(urlStore.statisticQuery, {
        ...
        .then(res => {
          if (res.ok) {
            ...
            let tmpData = {
              columns: ['单位', '总问题数', '一级单位', '二级单位', '三级单位'],
              rows: [],
              type: 'company'
            }
            ...
            res.body.data[0] && res.body.data[0].units.forEach(item => {
              tmpData.rows.push({
                '单位': item.unitName,
                '总问题数': item.resultCount,
                '一级单位': item.levelResultCount?item.levelResultCount.level1:0,
                '二级单位': item.levelResultCount?item.levelResultCount.level2:0,
                '三级单位': item.levelResultCount?item.levelResultCount.level3:0,
                unitCode: item.unitCode
              })
            })
    ...
    handleShowMethodChartMulti(level) {
      ...
      let unitCodes = this.selectedCompany.map(item => {
        ...
        .join(',')
      let methodIds = this.selectedMethods.map(item => item.id)
        .join(',')
          ...
          if (res.ok) {
            ...
            res.body.data.forEach(item => {
              ...
              item.units.forEach(unit => {
                tmpData.rows.push({
                  '单位': unit.unitName,
                  '总问题数': unit.resultCount,
                  '一级单位': unit.levelResultCount?unit.levelResultCount.level1:0,
                  '二级单位': unit.levelResultCount?unit.levelResultCount.level2:0,
                  '三级单位': unit.levelResultCount?unit.levelResultCount.level3:0,
                  methodId: item.methodId,
    ...
    handleAddAll(target, searchVal) {
      if (target == 'method') {
        if (searchVal) {
          let methodsToAdd = this.getAllMethods()
            .filter(item => (item.title.indexOf(searchVal) > -1 && !this.selectedMethods.some(seMethod => seMethod.id == item.id)))
    ...
    handleClickMethodChart(data, index) {
      this.drillingDataInfo.method = {
        methodId: this.selectedMethods.find(item => item.title == data.name)
          .id,
    ...
    handleExportDataByPcode(drillLevel) {
      ...
      let unitCodes
      if (drillLevel == 1) {
        methodIds = this.drillData.map(item => item.methodId)
          .join(',')
        ...
        methodIds = this.drillData2.map(item => item.methodId)
          .join(',')
